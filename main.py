...
from prepare_filestructure import (
    prepare_feature_calc_directory,
...
def main():
    parser = argparse.ArgumentParser(description="Clustering metadynamic calculation")

    parser.add_argument("topology", help="Path to the topology file [.pdb]", type=str)

    parser.add_argument("trajectory", help="Path to trajectory file [.xtc]", type=str)
    ...
    parser.add_argument(
        "weights_field",
        help="The name of the variable responsible for the weights in `weights`",
        type=str,
    )

    parser.add_argument("num_frames", help="Number of frames for clustering", type=int)

    parser.add_argument(
        "-lj",
        help="Path to Lennard-Jones energies file [GROMACS-compatible]",
        default=None,
        type=str,
    )
    ...
    assert parser.parse_args().topology.endswith(
        ".pdb"
    ), "The topology file must be in .pdb format"
    assert parser.parse_args().trajectory.endswith(
        ".xtc"
    ), "The trajectory file must be in .xtc format"
    with open(parser.parse_args().weights) as f:
        assert f.readline().startswith(
            "#! FIELDS"
        ), "The weighting coefficient file must be in a PLUMED-compatible format"
    with open(parser.parse_args().lj) as f:
        for _ in range(2):
            f.readline()
        assert (
            "GROMACS" in f.readline()
